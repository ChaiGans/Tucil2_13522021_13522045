...
def dnc_n_curve(control_points, depth, max_depth, points_list):
    if depth < max_depth:
        ...
        middle_of_middle_point = [find_midpoint(control_points[i], control_points[i + 1]) 
        ...
        while len(middle_of_middle_point) > 2:
            middle_of_middle_point = [find_midpoint(middle_of_middle_point[i], middle_of_middle_point[i + 1]) 
                                      for i in range(len(middle_of_middle_point) - 1)]
            middle_point_useful_left.append(middle_of_middle_point[0])
            middle_point_useful_right.insert(0, middle_of_middle_point[-1])
        ...
        if len(middle_of_middle_point) == 2:
            ...
            # When only one point remains, it is the new middle point
            new_middle_point = middle_of_middle_point[0]
...
def generate_bezier_dnc_n_curve(start_point, control_points, end_point, iterations):
    ...
    return [start_point] + bezier_curve_points + [end_point]
